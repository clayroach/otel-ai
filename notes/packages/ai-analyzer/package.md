...
class AIAnalyzerService extends Context.Tag("AIAnalyzerService")<
  AIAnalyzerService,
  {
    ...
    analyzeRealtimeStream: (dataStream: Stream.Stream<OTLPData, never, never>) => Stream.Stream<AnomalyResult, AnalysisError, never>
    ...
    correlateAnomalies: (anomalies: AnomalyResult[]) => Effect.Effect<CorrelationResult[], AnalysisError, never>
    ...
    return {
      analyzeRealtimeStream: (dataStream: Stream.Stream<OTLPData, never, never>) =>
        dataStream.pipe(
          ...
          Stream.groupedWithin(config.realtime.batchSize, Duration.seconds(config.realtime.windowSize)),
...
const AIAnalyzerLayer = Layer.effect(
  AIAnalyzerService,
  makeRealtimeAnalyzer
)
