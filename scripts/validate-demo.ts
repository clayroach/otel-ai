...
class DemoValidator {
  ...
  async validateDemoServices(): Promise<void> {
    ...
    try {
      ...
      const loadGenStatus = await this.execCommand(
        'cd demo/otel-demo-app && docker compose ps load-generator --format "{{.Status}}"'
      )
  ...
  async validateTelemetryFlow(): Promise<void> {
    ...
    try {
      ...
      if (tracesData.total_traces < 1000) {
        throw new Error(
          `Insufficient trace data: ${tracesData.total_traces} traces (expected 1000+)`
        )
      }
      ...
      if (tracesData.recent_traces < 50) {
        throw new Error(
          `No recent telemetry: ${tracesData.recent_traces} traces in last 2 minutes (expected 50+)`
        )
      }
  ...
  async validateServiceActivity(): Promise<void> {
    ...
    try {
      const statsData = await this.fetchJson(
        'http://localhost:4319/api/services/stats?since=5 MINUTE'
      )
      ...
      if (foundExpectedServices.length < 3) {
        throw new Error(
          `Expected demo services not found. Found: ${foundExpectedServices.join(', ')}`
        )
      }

      // Check for reasonable activity levels
      const totalTraces = services.reduce(
        (sum: number, service: any) => sum + parseInt(service.trace_count),
        0
      )
