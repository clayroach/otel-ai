...
export interface ResultAnalysisService {
  analyzeResults: (results: unknown[]) => Effect.Effect<ResultAnalysis, Error, never>
}

export const ResultAnalysisServiceTag =
  Context.GenericTag<ResultAnalysisService>('ResultAnalysisService')

// ========================
// Service Implementation
// ========================

export const ResultAnalysisServiceLive = Layer.succeed(
  ResultAnalysisServiceTag,
  ResultAnalysisServiceTag.of({
    analyzeResults: (results: unknown[]) =>
      Effect.gen(function* () {
        ...
        for (const columnName of columnNames) {
          const columnAnalysis = yield* analyzeColumn(
            columnName,
        ...
        // Detect patterns
        const patterns = yield* detectPatterns(columns, results)

        // Recommend chart type
        const { chartType, confidence, reasoning } = yield* recommendChart(columns, patterns)
