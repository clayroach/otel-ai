...
async function testModelDifferentiation() {
  ...
  for (const model of models) {
    ...
    try {
      ...
      console.log(`   - Metadata: selectedModel=${result.metadata.selectedModel}, llmModel=${result.metadata.llmModel}`)
  ...
  const insightCounts = Array.from(results.entries()).map(([model, result]) => 
    ({ model, count: result.insights.length })
  )
  ...
  if (results.has('local-statistical-analyzer')) {
    ...
    if (enhancedModels.length > 0) {
      ...
      const avgEnhancedInsights = enhancedInsightCounts.reduce((a, b) => a + b, 0) / enhancedInsightCounts.length
      
      if (statInsights < avgEnhancedInsights) {
        console.log(`✅ Statistical analyzer has fewer insights (${statInsights}) than enhanced models (avg: ${avgEnhancedInsights.toFixed(1)})`)
      } else {
