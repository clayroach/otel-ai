...
async function checkClickHouseData() {
  ...
  try {
    ...
    const response = await fetch('http://localhost:8123/?query=SELECT count(*) FROM otel.otel_traces', { headers });
    ...
    if (count > 0) {
      ...
      // Show recent traces
      const tracesResponse = await fetch('http://localhost:8123/?query=SELECT TraceId, ServiceName, SpanName, Duration, Timestamp FROM otel.otel_traces ORDER BY Timestamp DESC LIMIT 5 FORMAT JSON', { headers });
