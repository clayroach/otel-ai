...
import { Card, Row, Col, Statistic, Progress, Typography, Space, Tag, Alert, List } from 'antd'
          ...
          <Card size="small" title="📈 Rate" styles={{ body: { padding: '8px' } }}>
            <Statistic
              ...
              valueStyle={{
                fontSize: 16,
                color:
                  metrics.rateStatus === 2
                    ? '#ff4d4f'
                    : metrics.rateStatus === 1
                      ? '#faad14'
                      : '#1890ff'
              }}
            />
        ...
        <Col span={8}>
          <Card size="small" title="⚠️ Errors" styles={{ body: { padding: '8px' } }}>
            ...
            <Text type={metrics.errorRate > 5 ? 'danger' : 'secondary'} style={{ fontSize: 10 }}>
              Threshold:{' '}
              {metrics.errorStatus === 2 ? '>5%' : metrics.errorStatus === 1 ? '1-5%' : '<1%'}
            </Text>
          </Card>
        </Col>
        ...
        message={
          <div>
            <span style={{ fontSize: 12 }}>
              🤖 <strong>AI Analysis:</strong> {healthExplanation.summary}
            </span>
          </div>
        }
        type={
          healthExplanation.status === 'critical'
            ? 'error'
            : healthExplanation.status === 'warning'
              ? 'warning'
              : 'success'
        }
            ...
            renderItem={(metric) => (
              <List.Item style={{ padding: '4px 0' }}>
                <Space style={{ width: '100%' }}>
                  ...
                  <Tag
                    color={metric.status === 'critical' ? 'red' : 'orange'}
                    style={{ fontSize: 11 }}
                  >
                    {metric.value}
                  </Tag>
