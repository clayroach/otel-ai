...
const extractQueryDescription = (query: string): string => {
  ...
  if (whereMatch) {
    ...
    if (whereClause.includes('timestamp') || whereClause.includes('time')) {
      if (whereClause.includes('subtracthours') || whereClause.includes('subtracthours') || whereClause.includes('interval')) {
...
export const useAppStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      queryHistory: [],
      addToQueryHistory: (query: string) => set((state) => {
        const trimmedQuery = query.trim();
        if (!trimmedQuery || state.queryHistory.some(h => h.query === trimmedQuery)) return state;
