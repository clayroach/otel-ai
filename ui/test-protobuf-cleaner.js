...
mockServices.forEach(service => {
  ...
  console.log(`✅ Mock: "${service}" → "${result}" ${result === service ? '(unchanged)' : '(CHANGED!)'}`);
});
...
realServices.forEach((service, index) => {
  ...
  console.log(`${passed ? '✅' : '❌'} Real: "${service.substring(0, 50)}..." → "${result}" ${passed ? '' : `(expected "${expected}")`}`);
});
...
const edgeCases = [
  ...
  { input: '{"$typeName":"wrong.type","value":"test"}', expected: '{"$typeName":"wrong.type","value":"test"}' }
];

edgeCases.forEach(({ input, expected }) => {
  const result = cleanServiceName(input);
  const passed = result === expected;
  console.log(`${passed ? '✅' : '❌'} Edge: "${input}" → "${result}" ${passed ? '' : `(expected "${expected}")`}`);
});
